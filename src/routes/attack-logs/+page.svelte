<script lang="ts">
  ...
  interface AttackLong {
    attack_log: AttackLogEntry[];
  }
...
//   let AttackLogEntry: 
  ...
  onMount(async () => {
    try {
        // TODO
        // replace with attack log summary
        // fetch 
      const { data } = await invokeApiRequest<AttackLog>(
        "/attack-log",
        null,
        Method.GET,
        $user.token
  ...
  const fetchAttackLogs = async (worldId: string) => {
    if (!worldId) return;
...
<button on:click={printToken}>Get Token</button>

<p>Token Status: {authToken ? 'Loaded' : 'Not Available'}</p>
